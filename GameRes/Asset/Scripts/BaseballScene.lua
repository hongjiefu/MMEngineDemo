...
local BaseballScene = xe:Class(xe.Scene, {
    tag = "BaseballScene"
})
...
function BaseballScene:onStart()
    ...
    BaseballScene:PlayAnimation(self.world,"Baseball_Actor","Asset/Public/Baseball/Baseball_OP.seq",false,1,OpAniCallBack)

    --游戏结束界面，再来一局，按钮点击重置游戏数据
    local GameOverBtnNode = self.ScroeUIScene:GetChildByNameRecrusive("Btn_Image")
    GameOverBtnNode:AddTouchEventListener(function(node, type)
        print("-----------------------------------再来一局 重启游戏-----------------------------------")

        -- xe.Scheduler:scheduleOnce(function()
        --     xe.Director:GetInstance():PopScene();
        
        -- local scene = BaseballScene:new("com.wemomo.engine")
        -- xe.Director:GetInstance():PushScene(scene)
        -- end, xe.Scheduler.INTERVAL_NEXT_TICK)

    end)

    --UI控件监听事件
    local BtnNode = TouchUIScene:GetChildByNameRecrusive("TouchImage")
    BtnNode:AddTouchEventListener(function(node, type)
        if XUIWidget.XUI_BEGAN == type then
            ...
            if YPosHigh_0 <= self.FlyRolePos.y and self.FlyRolePos.y < YPosLow_0 then
                ...
                self.FlyScore = math.random (VarA_0, VarB_0) 
                ...
                BaseballScene:PlayAnimation(self.world,self.HitRoleActorName,self.Ani_Hit02,false,0.05,Anicallback)    
            ...
            elseif YPosHigh_1 <= self.FlyRolePos.y and self.FlyRolePos.y < YPosLow_1 then
                ...
                self.FlyScore = math.random (VarA_1, VarB_1) 
            ...
            elseif YPosHigh_2 <= self.FlyRolePos.y and self.FlyRolePos.y < YPosLow_2 then
                ...
                self.FlyScore = math.random (VarA_2, VarB_2) 
                ...
                BaseballScene:PlayAnimation(self.world,self.HitRoleActorName,self.Ani_HitMiss,false,1,Anicallback)    
            end
...
function BaseballScene:onTick(fDelta)
    ...
    if self.GameState == 9 and self.GameState9 == nil then
        ...
        self.FlyRole:SetMovement(GameState9_origin, GameState9_dest, GameState9_dt, GameState9_callback, GameState9_tickCallBack, GameState9_originVelocity)
        self.GameState9 = 1
    ...
    if self.GameState == 2 and self.GameState1 == nil then
        ...
        self.FlyRole:SetMovement(GameState1_origin, GameState1_dest, GameState1_dt, GameState1_callback, GameState1_tickCallBack, GameState1_originVelocity)
        self.GameState1 = 1
    ...
    if self.GameState == 4 and self.GameState4 == nil then
        ...
        self.FlyRole:SetMovement(GameState4_origin, GameState4_dest, GameState4_dt, GameState4_callback, GameState4_tickCallBack, GameState4_originVelocity)
        self.GameState4 = 1
